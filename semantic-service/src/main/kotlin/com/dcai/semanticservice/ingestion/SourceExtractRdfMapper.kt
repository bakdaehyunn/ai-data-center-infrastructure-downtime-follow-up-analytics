...
class SourceExtractRdfMapper {
    ...
    private class MappingContext(
        ...
        fun mapAsset(record: AssetSourceRecord) {
            ...
            record.criticalityLevel?.let { canonicalModel.add(asset, Dcai.hasCriticalityLevel, it) }
            record.operationalStatus?.let { canonicalModel.add(asset, Dcai.hasOperationalStatus, it) }
        ...
        fun mapImpact(record: ImpactSourceRecord) {
            ...
            record.mitigationState?.let { canonicalModel.add(impact, Dcai.hasMitigationState, it) }
            record.vendorState?.let { canonicalModel.add(impact, Dcai.hasVendorState, it) }
        ...
        fun mapEvidence(record: EvidenceSourceRecord) {
            ...
            record.telemetryStatus?.let { canonicalModel.add(evidence, Dcai.hasTelemetryStatus, it) }
            record.validationId?.let { canonicalModel.add(evidence, Dcai.hasValidationId, it) }
            record.validationStatus?.let { canonicalModel.add(evidence, Dcai.hasValidationStatus, it) }
